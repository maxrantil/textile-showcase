...
export const dataFetchers = {
   ...
   */
  async fetchOne<T>(query: string, params: Record<string, unknown>, options?: FetchOptions): Promise<T | null> {
...
export const preloadData = {
  ...
  async preloadHome(): Promise<void> {
    ...
    resilientFetch(queries.getDesignsForHome, {}, { cache: true })
      .catch(error => console.warn('Failed to preload home data:', error))
  },
  ...
  async preloadProject(slug: string): Promise<void> {
    ...
    resilientFetch(queries.getProjectBySlug, { slug }, { cache: true })
      .catch(error => console.warn(`Failed to preload project ${slug}:`, error))
  }
}
