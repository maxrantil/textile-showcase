...
export function useGalleryPreloader({ 
  ...
  useEffect(() => {
    if (!isMobile && designs.length > 1) {
      ...
      const adjacentUrls = [
        designs[prevIndex]?.image ? getOptimizedImageUrl(designs[prevIndex].image, { width: 800, quality: 80, format: 'webp' }) : null,
        designs[nextIndex]?.image ? getOptimizedImageUrl(designs[nextIndex].image, { width: 800, quality: 80, format: 'webp' }) : null
