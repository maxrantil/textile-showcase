...
import { RealUserMonitor } from './performance-monitoring'
import { CoreWebVitalsOptimizer } from './web-vitals-tracker'
...
export interface PerformanceBudgetConfig {
  ...
  // Real user metrics thresholds
  realUserMetrics: {
    minimumSampleSize: number
    acceptablePerformancePercentile: number // e.g., 0.75 for 75th percentile
    maxRegressionTolerance: number // percentage
  }
...
export class PerformanceBudgetValidator {
  ...
  private rumMonitor: RealUserMonitor | null
  private coreWebVitalsOptimizer: CoreWebVitalsOptimizer | null
  private dashboard: PerformanceDashboard | null
  ...
  async validatePerformanceBudget(
    ...
    const improvements: BudgetValidationResult['improvements'] = []
   ...
   */
  private async validateRealUserMetrics(): Promise<{
    violations: BudgetValidationResult['violations']
    improvements: BudgetValidationResult['improvements']
    sampleSize: number
  }> {
    const violations: BudgetValidationResult['violations'] = []
    const improvements: BudgetValidationResult['improvements'] = []
    ...
    try {
      ...
      const cwv = rumData.coreWebVitals as any // Type assertion for flexibility

      // LCP validation
      if (cwv?.lcp?.p75 > this.config.coreWebVitals.lcp) {
      ...
      // CLS validation
      if (cwv?.cls?.p75 > this.config.coreWebVitals.cls) {
      ...
      // INP validation
      if (cwv?.inp?.p75 > this.config.coreWebVitals.inp) {
   ...
   */
  private validateLighthouseScores(lighthouseResults: unknown): {
    violations: BudgetValidationResult['violations']
    improvements: BudgetValidationResult['improvements']
  } {
    const violations: BudgetValidationResult['violations'] = []
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const lhResults = lighthouseResults as any // Type assertion for flexibility

    for (const category of categories) {
      const current = lhResults.categories?.[category.key]?.score || 0
   ...
   */
  private async validateBundleSize(bundleStats: BundleStats): Promise<{
    violations: BudgetValidationResult['violations']
    improvements: BudgetValidationResult['improvements']
  }> {
    const violations: BudgetValidationResult['violations'] = []
  ...
  private generateRecommendations(
    ...
    const recommendations: string[] = []
    const violationsByCategory = violations.reduce(
      (acc, violation) => {
        acc[violation.category] = acc[violation.category] || []
        acc[violation.category].push(violation)
        return acc
      },
      {} as Record<string, BudgetValidationResult['violations']>
    )
   ...
   */
  generateBudgetReport(validationResult: BudgetValidationResult): string {
    const report = []
