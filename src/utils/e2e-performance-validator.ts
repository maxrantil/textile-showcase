...
import { BundleMonitor, DEFAULT_BUNDLE_BUDGET } from './bundle-monitor'
import {
...
export interface E2EPerformanceConfig {
  // Test scenarios
  scenarios: {
    name: string
    url: string
    device: 'mobile' | 'desktop' | 'tablet'
    networkThrottling: 'fast-3g' | 'slow-3g' | 'dsl' | 'no-throttling'
    userAgent?: string
...
export interface E2EValidationReport {
  ...
  scenarios: {
    [scenarioName: string]: {
      ...
      averages: {
        lighthouse: E2ETestResult['lighthouse']
        coreWebVitals: E2ETestResult['coreWebVitals']
        duration: number
      }
      failures: string[]
...
const DEFAULT_E2E_CONFIG: E2EPerformanceConfig = {
  scenarios: [
    {
...
export class E2EPerformanceValidator {
  ...
  constructor(config: Partial<E2EPerformanceConfig> = {}) {
    this.config = { ...DEFAULT_E2E_CONFIG, ...config }

    // Initialize validation components
    this.bundleMonitor = new BundleMonitor(DEFAULT_BUNDLE_BUDGET)
   ...
   */
  async runE2EValidation(): Promise<E2EValidationReport> {
   ...
   */
  private async runScenarioTests(
    scenario: E2EPerformanceConfig['scenarios'][0]
  ) {
  ...
  private async runLighthouseAudit(
    ...
    return new Promise<E2ETestResult['lighthouse']>((resolve, reject) => {
      const args = [
        scenario.url,
        '--output=json',
        '--quiet',
   ...
   */
  private extractCoreWebVitals(lighthouseResult: E2ETestResult['lighthouse']) {
   ...
   */
  private validateMetrics(metrics: {
    lighthouse: E2ETestResult['lighthouse']
    coreWebVitals: E2ETestResult['coreWebVitals']
    bundleSize: E2ETestResult['bundleSize']
  }) {
    const failures: string[] = []
  ...
  private async startServer() {
    return new Promise<import('child_process').ChildProcessWithoutNullStreams>(
      (resolve, reject) => {
   ...
   */
  private analyzeOverallResults(report: E2EValidationReport) {
   ...
   */
  private async generateReports(report: E2EValidationReport) {
    this.ensureOutputDirectory()
    ...
    for (const format of this.config.output.formats) {
      ...
      const filepath = path.join(this.config.output.reportDirectory, filename)
   ...
   */
  private generateMarkdownReport(report: E2EValidationReport): string {
...
`
   ...
   */
  private generateHTMLReport(report: E2EValidationReport): string {
...
<body>
   ...
   * Generate CSV report
   */
  private generateCSVReport(report: E2EValidationReport): string {
    const headers = [
      'Scenario',
      'Iteration',
      'Timestamp',
   ...
   */
  private logSummary(report: E2EValidationReport) {
  ...
  private ensureOutputDirectory() {
    if (!fs.existsSync(this.config.output.reportDirectory)) {
      fs.mkdirSync(this.config.output.reportDirectory, { recursive: true })
    }
  }

  private getNetworkRTT(throttling: string): number {
    switch (throttling) {
      case 'slow-3g':
        return 400
      case 'fast-3g':
  ...
  private getNetworkThroughput(throttling: string): number {
    switch (throttling) {
      case 'slow-3g':
        return 400
      case 'fast-3g':
