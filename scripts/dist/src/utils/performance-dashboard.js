...
exports.initializePerformanceDashboard = initializePerformanceDashboard
exports.getPerformanceDashboard = getPerformanceDashboard
exports.destroyPerformanceDashboard = destroyPerformanceDashboard
...
class PerformanceDashboard {
   ...
   */
  updateMetricSeries(series, value, timestamp) {
   ...
   */
  async sendAnalyticsAlert(rule, metric) {
   ...
   * Send webhook alert
   */
  async sendWebhookAlert(rule, metric) {
   ...
   * Send email alert
   */
  async sendEmailAlert(rule, metric) {
   ...
   */
  notifySubscribers(metricName, metric) {
    const callbacks = this.subscribers.get(metricName) || []
   ...
   */
  getMetricAnalysis(name, threshold) {
  ...
  identifyBottlenecks() {
    ...
    if (lcp && lcp.current > 1200) {
      bottlenecks.push({
        type: 'LCP',
        severity: lcp.current > 2500 ? 'critical' : 'high',
    ...
    if (cls && cls.current > 0.1) {
      bottlenecks.push({
        type: 'CLS',
        severity: cls.current > 0.25 ? 'critical' : 'high',
  ...
  checkPerformanceBudget() {
    const budget = performance_1.DEFAULT_PERFORMANCE_BUDGET
    const violations = []
  ...
  updateAlertRule(metric, updates) {
    ...
    if (index > -1) {
      this.alerts[index] = Object.assign(
        Object.assign({}, this.alerts[index]),
        updates
      )
 ...
 */
function initializePerformanceDashboard(config) {
  if (globalDashboard) {
 ...
 */
function destroyPerformanceDashboard() {
  if (globalDashboard) {
