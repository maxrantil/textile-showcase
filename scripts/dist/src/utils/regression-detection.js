...
exports.DEFAULT_REGRESSION_THRESHOLDS = exports.PerformanceRegressionDetector =
  void 0
...
class PerformanceRegressionDetector {
  ...
  calculateBaseline(history) {
    ...
    const calculateRobustMean = (values) => {
   ...
   * Analyze Performance Score regression
   */
  analyzePerformanceScore(current, baseline) {
   ...
   * Analyze Core Web Vitals regressions
   */
  analyzeCoreWebVitals(current, baseline) {
    const alerts = []
    const metrics = [
      {
        name: 'LCP',
   ...
   * Analyze Bundle Size regressions
   */
  analyzeBundleSize(current, baseline) {
    const alerts = []
    const metrics = [
      {
   ...
   */
  analyzeRealUserMetrics(current, baseline) {
    const alerts = []
   ...
   */
  calculateConfidence(changePercentage, type) {
   ...
   */
  async processRegressionAlerts(alerts) {
   ...
   */
  generateRegressionReport(alerts, metrics) {
    const report = []
...
exports.DEFAULT_REGRESSION_THRESHOLDS = {
  performanceScore: {
