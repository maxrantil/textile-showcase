...
import {
  ...
  type E2EPerformanceConfig,
} from '../src/utils/e2e-performance-validator'
  ...
  scenarios: [
    {
      ...
      networkThrottling: 'no-throttling' as const,
    ...
    },
    {
      ...
      networkThrottling: 'fast-3g' as const,
    ...
    },
    {
      ...
      device: 'mobile' as const,
      networkThrottling: 'slow-3g' as const,
      viewport: { width: 375, height: 667 },
    },
    {
...
async function runE2EValidation() {
  ...
  console.log(
    `📊 Testing ${CONFIG.scenarios?.length || 0} scenarios with ${CONFIG.testing?.iterations || 0} iterations each`
  )

  try {
    // Initialize validator
    const validator = new E2EPerformanceValidator(
      CONFIG as Partial<E2EPerformanceConfig>
    )
    ...
    console.log(
      `\n📄 Detailed reports saved to: ${CONFIG.output?.reportDirectory || './e2e-performance-reports'}`
    )
